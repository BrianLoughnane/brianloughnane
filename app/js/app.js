...
// ================================
	...
	function introExplode(num) {
		...
		introNumber.fadeIn().delay(700).hide(0);
		setTimeout(function() {
			blowUp(introNumber);
			introExplodeCounter++;
		}, 1000);
	...
	function introExplodeLast(num) {
		var introNumber = $('#introExplode span:nth-child('+num+')');
		introNumber.fadeIn().delay(1800).hide(0);
		// introExplodeCounter++;
		// setTimeout(function() {
	...
	function introRoll() {
		if (introFadeCounter < $('#introFades span').length) {
			...
			setTimeout(function() {	
				// blowUp($('#introFades span:last-child'));
				$('#introFades span:last-child').hide();
	...
	$('.skipIntro').on('click', function() {
		...
		introFadeCounter = 100;
		introExplodeCounter = 100;
		$("#main-blackout").finish();
		$('#main-blackout').hide();
		$('#baliRain')[0].pause();
		$('#explosionSound')[0].pause();
		$('#lightningSound')[0].pause();
		// $('.intro').hide();
		// $('.skipIntro').hide();
		setup();
	});
...
// ================================
	...
	$(document).on("click", '.toProjects', function(e) {
		e.preventDefault();
		e.stopPropagation();
	...
	$('#content-container').on("click", '.contact-text', function(e) {
		e.preventDefault();
		e.stopPropagation();
		debugger
...
// ================================	
	...
	function projectsSetup() {
		...
		$('.content').on('scroll', function() {
				...
				// // Modified for only having 2 sections (featured and thinkful):	

				if($('.content').scrollTop() >= featuredLocation -50 && $('.content').scrollTop() < thinkfulLocation -50) {
					highlightFeatured();
				} else if($('.content').scrollTop() >= thinkfulLocation -50) {
					highlightThinkful();
				}
		});
